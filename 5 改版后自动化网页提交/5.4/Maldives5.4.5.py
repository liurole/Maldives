...
# STEP 1，首先读取所需的paragraph.csv，detail.csv文件，被注释的两端分别用来检测是否运行成功
    ...
    pick = {}
    pick_temp = tables[int(index[0]) - 1]
    for key in pick_temp:
        pick[key] = str(pick_temp[key])
    
    for i in range(1, len(index)):
        pick_temp = tables[int(index[i]) - 1]
        for key in pick_temp:
            temp = str(pick_temp[key])    
            if temp != pick[key]:
                pick[key] += '/' + temp
    
    temp_version1 = pick['型号']
    temp = temp_version1.split('/')
    temp_version2 = temp[0]
    for i in range(1, len(temp)):
        temp_version2 += '，' + temp[i]
    ...
    context = {
        'MyTable' : [
            {'title' : '【产品】', 'type' : MyContent_type, 'abstract' : MyContent_abstract,
             'factory' : MyContent_factory, 'device' : MyContent_device, 'version' : temp_version2, 
