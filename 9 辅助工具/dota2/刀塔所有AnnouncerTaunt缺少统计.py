...
def read():
    # 读取Excel
    file = '刀塔所有AnnouncerTaunt名称.xlsx'
    ...
    # 创建一个空的 DataFrame  
    df_own = pd.DataFrame(columns=['拥有', '类型'])  
    
    for index in result_web.index:
        temp = result_web['物品名称'][index].lower()
        if temp in name_new:
            df_own.loc[index] = [1, result_web['类型'][index]]
        else:
            df_own.loc[index] = [0, result_web['类型'][index]]
...
if __name__ == '__main__':
    ...
    result.to_excel("刀塔所有AnnouncerTaunt缺少统计.xlsx", index=False)     # write data to excel
