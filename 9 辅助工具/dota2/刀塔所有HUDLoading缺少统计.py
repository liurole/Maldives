...
def hud():
    ...
    for index in result_web.index:
        ...
        if temp in name_new:
            df_own.loc[index] = [1, 0]
        else:
            df_own.loc[index] = [0, 0]
...
def loading():
    ...
    for index in result_web.index:
        temp = result_web['物品名称'][index].lower()
        if temp in name_new:
            df_own.loc[index] = [1, 1]
        else:
            df_own.loc[index] = [0, 1]
...
if __name__ == '__main__':
    ...
    result.to_excel("刀塔所有HUDLoading缺少统计.xlsx", index=False)     # write data to excel
